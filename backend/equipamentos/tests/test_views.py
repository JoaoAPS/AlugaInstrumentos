...
def test_equipamento_list_view_basic(list_url, userClient, factory):
    """Testa a list view dos equipamentos sem GET params"""
    mixer.blend(Equipamento)
    mixer.blend(Equipamento)
    mixer.blend(Equipamento)

    equipamentos = Equipamento.objects.all().order_by('title')
    res = userClient.get(list_url)
    serializer = EquipamentoSerializer(
        equipamentos,
        many=True,
        context={'request': factory.get(list_url)}
    )
...
def test_equipamento_list_view_optional_params(list_url, userClient, factory):
    ...
    equipamentos = [e2, e1]
    serializer = EquipamentoSerializer(
        equipamentos, many=True, context={'request': factory.get(list_url)}
    )
...
def test_equipamento_list_view_filter_available_dates(
    list_url, userClient, factory
):
    ...
    serializer = EquipamentoSerializer(
        [e3], many=True, context={'request': factory.get(list_url)}
    )
    res = userClient.get(
...
def test_equipamento_create_view_mandatory_parameters_only(
    list_url, adminClient, tmp_image
):
...
def test_equipamento_create_view_all_parameters(
    list_url, adminClient, tmp_image
):
...
def test_equipamento_full_update_view_successful(
    ...
    assert res.status_code == status.HTTP_200_OK
    assert equipamento.title == payload['title']
...
def test_equipamento_update_view_unexisting_id(detail_url, adminClient):
    ...
    res = adminClient.put(detail_url(77), {'title': 'N', 'price_per_day': 1.4})
    assert res.status_code == status.HTTP_404_NOT_FOUND

    res = adminClient.patch(
        detail_url(7), {'title': 'N', 'price_per_day': 1.4}
    )
    assert res.status_code == status.HTTP_404_NOT_FOUND
